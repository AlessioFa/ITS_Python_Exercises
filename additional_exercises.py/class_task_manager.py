...
class TaskMAnager:
    ...
    def create_task(self, task_id: str, description: str) -> dict[str, dict[str, str | bool]]:
        ...
        if task_id in self.tasks:
            ...
            # per costruzione è meglio aggiungere task_id: poichè l esercizio vuole un nuovo dizionario
            return {task_id: self.tasks[task_id]}

        else:
            # se l esercizio avesse voluto un eccezione : raise KeyError("Chiave non presente!")
    ...
    def remove_task(self, task_id: str) -> dict | str:
        ...
        if task_id not in self.tasks:
            ...
            # si potrebbe usare .popitem che restituisce una tupla , la aprte a sinistra della tupla èp la chiave associata al valore nel dizionario , la aprte di destra è il valore associato a quella chiave
